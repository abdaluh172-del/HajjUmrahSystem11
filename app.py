...
ADMIN_NAME = "عبدالله الحربي"          # display name of the fixed admin
...
def init_db():
    ...
    if admin_row is None:
        ...
        if legacy:
            conn.execute(
                ...
                (ADMIN_NAME, ADMIN_EMAIL, generate_password_hash(ADMIN_PASSWORD), legacy[0]),
            )
    ...
    # Always enforce the configured display name for the fixed admin.
    conn.execute("UPDATE users SET name=? WHERE lower(email)=?", (ADMIN_NAME, ADMIN_EMAIL))
